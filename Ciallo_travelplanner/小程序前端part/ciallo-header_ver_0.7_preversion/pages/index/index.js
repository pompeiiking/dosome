...
Page({
  ...
  onSubmit() {
    // 调用 submitPreferences 方法处理请求和跳转逻辑
  ...
  onSelectActivity(e) {
    ...
    // 生成activities_map
    let activities_map = {};
    Object.keys(interestFeatures).forEach(interest => {
      const selected = interestFeatures[interest].selected_tags || [];
      if (selected.length > 0) {
        activities_map[interest] = selected;
      }
    });
    this.setData({
      'form.preferences.travel_preferences.activities_map': activities_map
    });
  ...
  submitPreferences() {
    ...
    // 清理冗余advanced_options
    if (this.data.form.preferences.advanced_options) {
      delete this.data.form.preferences.advanced_options;
    }
    const rawData = this.data.form;
    console.log("发送到后端的原始数据:", JSON.stringify(rawData, null, 2));
    wx.showLoading({
        title: '正在生成推荐...',
    });
    request.sendTravelPlan(rawData)
      .then(res => {
        wx.hideLoading();
        console.log("收到后端响应:", res);
        // 检查后端返回数据是否有效
        if (res && res.paths && res.paths.length > 0) {
          // 数据有效，跳转到 overview 页面
          wx.navigateTo({
