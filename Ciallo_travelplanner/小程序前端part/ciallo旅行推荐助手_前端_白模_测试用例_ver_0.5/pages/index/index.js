...
Page({
  data: {
    ...
    // 新增：初始化兴趣选项
    preferenceOptions: [
      { label: "自然风光", icon: "/assets/icons/山_mountain.svg", selected: false },
      { label: "历史人文", icon: "/assets/icons/城门_city-gate.svg", selected: false },
      { label: "美食探店", icon: "/assets/icons/叉子勺子_fork-spoon.svg", selected: false },
      { label: "亲子乐园", icon: "/assets/icons/童车_steoller.svg", selected: false },
      { label: "冒险运动", icon: "/assets/icons/运动_sport.svg", selected: false },
      { label: "摄影打卡", icon: "/assets/icons/相机_camera.svg", selected: false },
      { label: "购物休闲", icon: "/assets/icons/购买_buy.svg", selected: false },
      { label: "小众体验", icon: "/assets/icons/巫师帽_sorcerer-hat.svg", selected: false }
    ],
    startY: 0,
    translateY: 0,
    progress: 0,
    ...
    contentTranslateY: 0,
    showWelcome: true,
    bgColor: 'linear-gradient(180deg, #e6f3ff 0%, #fff 60%)',
    headerOpacity: 0,
    reviewPreferencesFlatList: [] // 新增：用于复查页面直接显示的数据列表
  },
  onLoad() {
    // 测试连接
    this.testConnection();
  ...
  onSelectActivity(e) {
    ...
    console.log('活动选择后，更新的 selected_tags:', interestFeatures[interestKey].selected_tags);

    // 直接更新 reviewPreferencesDisplayList
    let currentReviewList = [...this.data.reviewPreferencesFlatList];
    const existingIndex = currentReviewList.findIndex(item => item.interest === interestKey);

    if (selectedTags.length > 0) {
        if (existingIndex > -1) {
            // 如果已存在该兴趣，更新其活动列表
            currentReviewList[existingIndex].activities = selectedTags;
        } else {
            // 如果不存在该兴趣且有选中活动，添加新项
            currentReviewList.push({
                interest: interestKey,
                activities: selectedTags
            });
        }
    } else {
        // 如果没有选中活动且已存在该兴趣，移除该项
        if (existingIndex > -1) {
            currentReviewList.splice(existingIndex, 1);
        }
    }

    this.setData({
        reviewPreferencesFlatList: currentReviewList
    });
    console.log('活动选择后，更新的 reviewPreferencesFlatList:', this.data.reviewPreferencesFlatList);
  },
  onNextStep() {
    // 步骤校验（可根据实际需要细化）
    ...
    // 当从步骤4进入步骤5时，准备 reviewSummary 数据
    if (this.data.currentStep === 4) {
      // 直接使用 reviewPreferencesFlatList 构建 reviewSummary.preferences
      const reviewSummaryPreferences = this.data.reviewPreferencesFlatList;

      this.setData({
        currentStep: this.data.currentStep + 1,
        reviewSummary: { // 将总结数据存入 reviewSummary
          preferences: reviewSummaryPreferences, // 使用 reviewPreferencesFlatList
          pace: this.data.form.preferences.travel_preferences.selected_pace,
          budget: this.data.form.preferences.budget.selected_type
          // 可以在这里添加其他需要复查的基本信息、行程信息等
        }
      });
      return; // 步骤跳转并设置数据后返回
    }
  ...
  submitPreferences() {
    ...
    // 构建activities_map，现在只包含选中的次级偏好，并保持顺序
    ...
    selected_interests.forEach(key => {
      // 直接获取按选择顺序排列的selected_tags
      const selectedTags = interest_features[key]?.selected_tags || [];
      // 将选中的标签列表作为值赋给对应的兴趣key
      activities_map[key] = selectedTags;
    });

    // 组装完整JSON
    const payload = {
      basic_info: this.data.form.basic_info,
      trip_requirements: this.data.form.trip_requirements,
      preferences: {
        travel_preferences: {
          selected_interests,
          activities_map, // 使用新的activities_map结构
      ...
      advanced_options: this.data.form.advanced_options
    };

    console.log('步骤5进入时，interest_features 数据:', this.data.form.preferences.travel_preferences.interest_features);
    console.log('发送到后端的JSON数据:', payload);

    wx.showLoading({
      title: '提交中...'
    });
    request.sendTravelPlan(payload)
      .then(res => {
        wx.hideLoading();
        // 存储结果到本地缓存
        wx.setStorageSync('travelResult', res);
        // 跳转到结果页面
        wx.navigateTo({
  ...
  onTouchStart(e) {
    if (!this.data.showWelcome && this.data.progress >= 1) return;
    if (e.touches) {
  ...
  onTouchMove(e) {
    if (!this.data.showWelcome && this.data.progress >= 1) return;
    let moveY = e.touches && e.touches.length > 0 ? e.touches[0].clientY : (e.changedTouches && e.changedTouches.length > 0 ? e.changedTouches[0].clientY : null);
    ...
    if (deltaY < -30) {
      ...
      let progress = Math.min(absY / MAX_DISTANCE, 1);
      let imgScale = lerp(1, 0.5, progress);
      let imgTop = lerp(80, 10, progress);
      let contentTranslateY = lerp(0, -300, progress);
      let welcomeOpacity = lerp(1, 0, progress);
      let mainOpacity = lerp(0, 1, progress);
      let headerOpacity = progress > 0.7 ? (progress - 0.7) / 0.3 : 0;
      this.setData({
        translateY: deltaY,
        progress,
        imgScale,
